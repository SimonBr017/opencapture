...
class FindFooter:
    ...
    def process(self, regex, text_as_string):
        ...
        if len(array_of_data) > 0:
            self.log.info('array of data : ' + str(array_of_data))
    ...
    def run(self, text_as_string=False):
        ...
        #test ajout condition si ht et vat amouunt sans ttc
        #if total_ht and vat_amount and not total_ttc:
        #    self.log.info('Total HT : [' + total_ht + '] and VAT Amount : [' + vat_amount + '], Found')
        #    ht = self.return_max(total_ht)[0]
        #    vat = vat_amount
        #    if 'from_position' in total_ht and total_ht['from_position']:
        #        ht = total_ht[0]
		...
		#if only ht and vat rat found calcul ttc
        if total_ht and vat_rate and not total_ttc:
            ht = self.return_max(total_ht)[0]
            vat = self.return_max(vat_rate)[0]
            self.log.info('Hors Tax :' + str(ht) + ' vat rate :' + str(vat))
            if 'from_position' in total_ht and total_ht['from_positiion']:
                ht = total_ht[0]
            total_ttc = [float("%.2f" % (float(ht) * float(1 + (float(vat) / 100)))), (('', ''), ('', ''))]

        #if only total TTc found, set automaticly vate rat at 0% if ttc < 200 or 20% if ttc >= 200
        if total_ttc and not total_ht and not vat_rate:
            loop_ttc = 0
            self.log.info('Footer informations found : Only TTC vat rat to control')
            if loop_ttc == 0:
                loop_ttc += 1
                return False
            else:
		...
		#if only total HT found, set automaticly vate rat at 0% if ht < 200 or 20% if ht >= 200 
        if total_ht and not total_ttc and not vat_rate:
            loop_ht = 0
            self.log.info('Footer informations found : Only HT vat rat to control')
            if loop_ht == 0:
                loop_ht += 1
                return False
            else:
            ...
            if 'from_position' in total_ht and total_ht['from_position']:
                ht = total_ht[0]
            total_ttc = [float("%.2f" % (float(ht) * float(1 + (float(percentage) / 100)))), (('', ''), ('', ''))]
