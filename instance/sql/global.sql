...
INSERT INTO "regex" ("regex_id", "lang", "label", "content") VALUES ('invoiceRegex', 'fra', 'Numéro de facture', '(?i)(?P<r1>=|commande\s*|facture\s*|ticket\s*|N#\s*|(NUM(E|é)RO|NUM|N(O|°|º|R.)?\s*(DE)?\s*(LA)?\s*(FaCTURE|PI(E|È)CE|DOCUMENT)(\s*:)?\s*(?(r1)()|(NUM(é|E)RO|num|N(O|°|º|#|R)?))(\s*:)?)|(FaCTURE(DATE)?)\s*(ACQUIT(T)?(E|É)E)?\s*(:|#){1}).*');
...
INSERT INTO "regex" ("regex_id", "lang", "label", "content") VALUES ('noRatesRegex', 'fra', 'Montant HT', '(?i)(?P<r1>MONTANT\s*(HT\D*)?|(SOUS(-|\s+)?)TOTAL|VAT\s*BASE|VALEUR\s*(BRUTE|POSITIONS|TOTALE)|IMPOSABLE|PRIX\s*HT\D*\s*(TOTAL)?|TOTAL\s*HT\D*\s*|TOTAL\s*(ORDRE|INTERM(E|É)DIAIRE)|BASE\s*TOTAL)?\s*(:\s*|EN)?(€|EUROS|EUR)?\s*(?(r1)()|(\()?((H(\.)?T(\.)?(V(\.)?A(\.)?)?|HORS TVA|(EXCL|BASE)\s*(\.)?\s*TVA|HORS\s*TAXES\s*\D*|TOTAL\s*INTERM(É|E)DIAIRE))(\))?){1}\s*(:)?(€|EUROS|EUR)?\s*([0-9]*(\.?\,?\s?)[0-9]+((\.?\,?\s?)[0-9])+|[0-9]+)\s*(€|EUROS|EUR)?|([0-9]*(\.?\,?\s?)[0-9]+((\.?\,?\s?)[0-9])+|[0-9]+)\s*(€)?\s*(HT)');
INSERT INTO "regex" ("regex_id", "lang", "label", "content") VALUES ('allRatesRegex', 'fra', 'Montant TTC', '(?i)(?P<r1>MONTANT\s*(NET)?\s*(TOTAL)?\s*(à|A)?\s*(PAYER)?\s*(TTC)?\s*|TOTAL\s*(TTC\D*)?(NET)?|PRIX\s*(TOTAL)?\s*(à|A)?\s*(PAYER)?\s*(TTC)?\s*|NET\s*(à|A)?\s*(PAYER)?\s*)?(:\s*|EN)?(€|EUROS|EUR)?(?(r1)()|(\()?((T(\.)?T(\.)?(C(\.)?)\D*|TOTAL\s*))(\))?){1}\s*(:)?(€|EUROS|EUR)?\s*([0-9]*(\.?\,?\s?)[0-9]+((\.?\,?\s?)[0-9])+|[0-9]+)\s*(€|EUROS|EUR)?|([0-9]*(\.?\,?\s?)[0-9]+((\.?\,?\s?)[0-9])+|[0-9]+)\s*(€)?\s*(TTC)');
INSERT INTO "regex" ("regex_id", "lang", "label", "content") VALUES ('vatRateRegex', 'fra', 'Taux de TVA', '(?i)(TVA|%)\s*(5(?:\.|,)5|19(?:\.|,)6|(6|10|12|20)(?:[.,]0{1,3})?)|(5(?:\.|,)5|19(?:\.|,)6|(6|10|12|20)(?:[.,]0{1,3})?)(\s*%)');
INSERT INTO "regex" ("regex_id", "lang", "label", "content") VALUES ('vatAmountRegex', 'fra', 'Montant TVA', '(?i)((MONTANT|TOTAL)\s*TVA(\s*[0-9.,]*\s*%)?|TVA\s*[0-9.,]*\s*%|TVA\s*[0-9.,]*\s*%?\s*SUR\s*[0-9.,]*\s*[A-Z\s'']*|%\s*TVA)\s*(€|EUROS|EUR)?.*');
